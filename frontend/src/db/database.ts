...
export interface HvacService {
  freonDate: string | null;   // ISO YYYY-MM-DD
  leakTested: boolean;
}
...
export async function initDatabase() {
  ...
  // Migration: HVAC service columns
...
// These functions are used by the online sync feature. Unlike importData
// (which is INSERT OR IGNORE for a 3-table subset), the sync-replace path
// wipes and reinserts EVERY table so cloud state fully overrides local.
  ...
  for (const s of snap.services) {
    await db.runAsync(
      `INSERT INTO services (
         id, vehicle_id, customer_id, service_description, additional_info, cost, is_paid,
      ...
      [
        ...
        s.cost, s.is_paid ? 1 : 0, s.service_date, s.created_at,
